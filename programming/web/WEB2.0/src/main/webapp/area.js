...
function drawCircle(r) {
    ...
    // Изменяем угол с нижнего левого на верхний левый
    ctx.arc(startX, startY, 0.5*r * oneCell, Math.PI, Math.PI * 1.5, false);
    ctx.closePath();
    ctx.fill();
}
...
function drawRect(r) {
    ...
    ctx.moveTo(startX, startY);
    ctx.fillStyle = fillStyle;
    // Переносим прямоугольник в правый верхний квадрант
    ctx.rect(startX, startY - r * oneCell, r * oneCell, r * oneCell);
    ctx.closePath();
    ctx.fill();
}
...
function drawTriangle(r) {
    ...
    ctx.moveTo(startX, startY);
    ctx.fillStyle = fillStyle;
    // Переносим треугольник в левый нижний квадрант
    ctx.lineTo(startX - 5 * halfCell, startY);
    ctx.lineTo(startX, startY + r * oneCell);
    ctx.closePath();
    ctx.fill();
}
...
drawAxis();
drawCoords(5);
dotx.forEach(element => {
    console.log(element,doty[dotx.indexOf(element)],dotcolor[dotx.indexOf(element)]);
    drawPoint(element,doty[dotx.indexOf(element)],dotcolor[dotx.indexOf(element)]);
});
